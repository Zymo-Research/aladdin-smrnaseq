...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        ...
        for f in self.find_log_files('Trim_Galore/logs', filehandles=True):
            self.parse_trim_galore_logs(f)

        for f in self.find_log_files('Trim_Galore/trimmedfastq', filecontents = False, filehandles=True):
            self.parse_trim_galore_fastq(f)
    ...
    def parse_trim_galore_fastq(self, f):
        s_name = f['fn']
        if s_name.endswith('_trimmed_fastqc.zip'):
            s_name = s_name[:-19]
            try:
            ...
            with fqc_zip.open(os.path.join(d_name, 'fastqc_data.txt')) as fh:
                ...
                fn_search = re.search("%GC\t(\d+)", r_data)
                if fn_search != None:
                    self.trim_galore_data[s_name]['trimmed_gcperc'] = int(fn_search.group(1))
    ...
    def trim_galore_general_stats_table(self):
        ...
        headers['trimmed_gcperc']  = {
            'title': '% Trimmed GC',
            'description': '% GC Content After Fastq Trimming by Trim Galore',
            'namespace': 'Trim_Galore',
            'max': 100,
            'min': 0,
            'suffix': '%',
