...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        ...
        for f in self.find_log_files("plot_gene_heatmap", filehandles=True):
            ...
            if len(parsed_data):
                if "isomirs" in f["fn"].lower():
                    self.plot_heatmap_data[f["fn"]] = parsed_data
                elif "deseq2" in f["fn"].lower():
                    parsed_data = parsed_data.drop("gene_name", axis = 1)
                    self.plot_heatmap_data[f["fn"]] = parsed_data
                else:
                    log.debug("Heatmap data does not contain isomirs or deseq in filename {}".format(f["fn"]))
            else:
